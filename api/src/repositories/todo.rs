...
impl TodoRepository for TodoRepositoryForDb {
    async fn create(&self, payload: CreateTodo) -> anyhow::Result<TodoEntity> {
        ...
        let row = sqlx::query_as::<_, TodoFrowmRow>(
            r#"
...
mod test {
    ...
    async fn crud_scenario() {
        ...
        .fetch_all(&pool)
        .await
        .expect("[delete' todo_lables fetch error]");
...
pub mod test_utils {
    ...
    impl TodoRepositoryForMemory {
        ...
        fn resolve_lables(&self, labels: Vec<i32>) -> Vec<Label> {
    ...
    impl TodoRepository for TodoRepositoryForMemory {
        async fn create(&self, payload: CreateTodo) -> anyhow::Result<TodoEntity> {
            ...
            let labels = self.resolve_lables(payload.labels);
        ...
        async fn update(&self, id: i32, payload: UpdateTodo) -> anyhow::Result<TodoEntity> {
            ...
            let labels = match payload.labels {
                Some(label_ids) => self.resolve_lables(label_ids),
